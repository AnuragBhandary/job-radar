...
/**
 * Renders a {@link ResumeSelection.Selected} resume to print-ready HTML.
 *
 * <p>HTML rather than LaTeX or docx, because headless Chromium prints it and a
 * second document toolchain is a second thing to install and keep working.
 *
...
public class ResumeRenderer {
    ...
    private final Applicant profile;

    public ResumeRenderer(Applicant profile) {
        this.profile = profile;
    }

    public String toHtml(ResumeSelection.Selected resume) {
        ...
        if (notEmpty(resume.skills())) {
            ...
            for (ResumeSource.SkillGroup group : resume.skills()) {
        ...
        if (notEmpty(resume.experience())) {
            ...
            for (ResumeSource.Job job : resume.experience()) {
                ...
                // The arrangement, printed rather than left for a background
                // check to discover.
        ...
        if (notEmpty(resume.projects())) {
            ...
            for (ResumeSource.Project project : resume.projects()) {
                html.append("<div class=\"entry\">")
        ...
        if (notEmpty(resume.education())) {
            ...
            for (ResumeSource.Education degree : resume.education()) {
                html.append("<div class=\"entry\">")
    ...
    private void header(StringBuilder html, String headline) {
        Applicant.Contact contact = profile.contact();
        html.append("<h1>").append(escape(profile.name().full().toUpperCase(java.util.Locale.ROOT))).append("</h1>");
    ...
    private static void bullets(StringBuilder html, List<ResumeSource.Bullet> bullets) {
        if (bullets == null || bullets.isEmpty()) {
            return;
        }
        html.append("<ul>");
        for (ResumeSource.Bullet bullet : bullets) {
